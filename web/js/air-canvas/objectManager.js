...
// window.gsap is loaded as a global UMD script before this module runs
...
export class ObjectManager {
    ...
    return new Promise((resolve) => {
      gsap.delayedCall(0.3, () => {
        gsap.to(obj.mesh.scale, {
          x: obj.scale, y: obj.scale, z: obj.scale,
  ...
  update(deltaTime, elapsedTime) {
    for (const obj of this.objects) {
      ...
      if (obj.squishAmount > 0) {
        ...
        obj.mesh.scale.set(obj.scale * expand, obj.scale * squish, obj.scale * expand);
      }
    }
  }
  ...
  grabObject(obj) {
    ...
    gsap.to(obj.mesh.scale, {
      x: obj.scale * 1.1, y: obj.scale * 1.1, z: obj.scale * 1.1,
      duration: 0.15,
      ease: 'power2.out'
    });
  }
  ...
  releaseObject(obj) {
    ...
    for (const other of this.objects) {
      if (other.id !== obj.id) this.findBalancedPosition(other);
    }
  }
  ...
  async removeObject(obj, fade = false) {
    ...
    return new Promise((resolve) => {
      ...
      if (fade) {
        ...
        gsap.to(obj.mesh.scale, {
          x: obj.scale * 1.3, y: obj.scale * 1.3, z: obj.scale * 1.3,
          duration: 0.1,
          ease: 'power2.out',
          onComplete: () => {
            gsap.to(obj.mesh.scale, {
  ...
  async clearAll() {
    ...
    const promises = snapshot.map((obj, i) =>
      new Promise((resolve) => {
        gsap.delayedCall(i * 0.1, async () => {
          if (this.objects.includes(obj)) {
            await this.removeObject(obj, true);
          }
          resolve();
        });
      })
  ...
  selectObject(obj) {
    gsap.to(obj.mesh.scale, {
      x: obj.scale * 1.15, y: obj.scale * 1.15, z: obj.scale * 1.15,
      duration: 0.15,
      ease: 'power2.out',
      onComplete: () => {
        gsap.to(obj.mesh.scale, {
