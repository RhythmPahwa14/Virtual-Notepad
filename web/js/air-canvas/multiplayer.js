// window.Peer is loaded as a global UMD script before this module runs
...
export class Multiplayer {
  ...
  handleConnection(conn) {
    ...
    conn.on('data', (data) => {
      this.emitEvent(data);
      if (this.isHost) this.broadcastExcept(data, peerId);
    });
  ...
  broadcastExcept(event, excludePeerId) {
    this.connections.forEach((conn, peerId) => {
      if (conn.open && peerId !== excludePeerId) conn.send(event);
    });
  }
